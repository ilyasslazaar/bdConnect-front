...
class Restrictions extends Component {
  ...
  changeOrderBy = column => {
    ...
    listFilter.page = 1;
    this.props.getRestrictions(listFilter);
  };
  changePageSize = size => {
    const { listFilter } = this.props;
  ...
  onDeleteItems = isConfirmed => {
    if (this.state.selectedItems.length > 0) {
      if (isConfirmed) {
        ...
        this.props.deleteRestrictions(ids, () => {
          this.createNotif("restrictions.deleted-success-message");
  ...
  onSubmitItem = box => {
    ...
    const successCallback = () => {
      ...
      this.createNotif("restrictions.add-success-message");
      this.toggleModal();
    };
    const errorCallback = () => {
    ...
    box.boxName = box.boxName.number;
    box.userName = box.userName.username;

    this.props.postRestrictions(box, successCallback, errorCallback);
  };
    ...
    if (this.props.listError) {
      return (
        <div className="col-12 text-center h6">
          <IntlMessages id="restrictions.error-gettings" />
        </div>
      );
    }
    return false;
  ...
  render() {
    ...
    return (
      <Fragment>
        <DataListContainer
          title={"menu.restrictions"}
          addNewText="restrictions.Add-Restrictions"
        ...
        <ConfirmationModal
          ...
          content={"restrictions.confirm-delete"}
...
const mapStateToProps = ({ appData }) => ({
  listData: appData.restrictions.data,
  listLoading: appData.restrictions.loading,
  listError: appData.restrictions.error,
  listFilter: appData.restrictions.filter
});

export default connect(
  mapStateToProps,
  actions
)(injectIntl(mouseTrap(Restrictions)));
